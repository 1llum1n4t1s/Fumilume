...
- 開いている文書を見渡しやすく、よく使う項目を先頭へピン留めできる垂直タブと、見出し・ブックマークへ切り替えられる左パネル
- UTF-8、UTF-8 BOM、UTF-16 LE／BE、UTF-32 LE／BE、Shift_JIS、EUC-JP、ISO-2022-JP と、CRLF、LF、CR の維持
- 正規表現対応の検索・置換、指定行への移動、行ブックマーク
- フォルダをまたいだ全文検索と、結果一覧からその行へ飛ぶタグジャンプ
- C#、JSON、XML、Markdown など拡張子に応じた構文の色分け（ライト・ダーク両対応）。C# と JSON は括弧をネストの深さごとに色分けし、カーソル脇の括弧と対応相手を強調表示
...
- CSV をExcel風の行番号・列名・罫線付きの表でプレビュー。「プレビュー」ボタンまたは `Ctrl+Shift+M` で切り替えます。セルをダブルクリック、または選択して `Enter` / `F2` で値を修正し、「適用」または `Ctrl+Enter` で反映、`Esc` で取り消します。入力欄では `Alt+Enter` で改行、`Enter` で適用して下へ、`Tab` / `Shift+Tab` で適用して左右へ移動します。適用後は通常の保存・元に戻す・やり直しを使えます。先頭行もデータとして表示し、先頭のゼロや数式の文字列をそのまま保持します
  - 未適用の入力は表示・タブの切り替えで取り消されます。修正は適用してから `Ctrl+S` で保存してください
  - 行番号・列名をクリックすると行・列全体を選択できます。`Shift`＋クリックで連続する複数行・列を選び、`Ctrl+C` でExcelなどに貼り付けられるタブ区切り形式でコピーできます
  - セルはクリック、`Shift`＋クリック、ドラッグで範囲選択できます。矢印や `Tab` で移動し、`Shift`＋矢印で範囲を広げられます。セル内の文字だけを選ぶ場合は `Alt`＋ドラッグを使います
  - 選択範囲を `Ctrl+C` / `Ctrl+X` でコピー／切り取り、選択範囲の左上へ `Ctrl+V` でExcelの表を貼り付けられます。足りない行・列は補完します。`Delete` / `Backspace` は値だけを消し、`Ctrl+D` は最上行、`Ctrl+R` は最左列の値を範囲内へ繰り返します。セルの右クリックからも操作できます
  - 見出しを右クリックすると、選択した行・列の前後への追加、削除、コピーを実行できます。複数行・列の選択中は同じ数だけ追加します。空のCSVにも行・列を追加できます。末尾への追加はコマンドパレットからも実行できます
  - 列見出しを右クリックして、選択した1列を基準に全行を昇順・降順へソートできます。先頭行を見出しとして固定する操作も選べます。数値は数値順、文字は英字の大小を区別しない文字列順に並べ、空欄は常に末尾、同じ値の行は元の順序を保ちます。列指定のソートはコマンドパレットからも利用できます（列番号を入力するためマクロ記録の対象外）
  - 列見出しの右端、行見出しの下端をドラッグして列幅・行高を調整できます。サイズは開いているタブごとに保持し、CSVファイルには保存しません
  - プレビューは先頭1,000行・100列まで表示し、超過時は画面に案内します。上限外に追加した行・列は「編集へ戻る」で確認・編集してください。追加位置やファイル自体の内容は変更・省略しません
  - 閉じていない引用符がある場合は警告します。「編集へ戻る」で引用符を修正してから表の操作を再試行してください。大きなCSVの解析・コピー・範囲編集・行列操作中も画面操作を継続できます。処理中に本文や選択を変えた場合は操作を中止します
  - 列の追加・削除・コピーは表示上限より後ろの行にも適用します。行コピーも表示上限より右の列を含みます
  - コピー・範囲編集・行列操作は、本文と貼り付け内容および結果が各8,388,608文字（UTF-16単位）、解析するセル数と編集後のセル数が各1,048,576個までです。超過時は内容を変更せず案内します。「編集へ戻る」で編集してください
  - CSV表示中はテキスト行の削除・複製などを無効にします。テキスト操作を含むマクロは編集画面へ切り替えて再生します
- PDF のページ表示、前後移動、幅・高さに合わせた表示、25～400% の手動拡大・縮小
- 行番号、折り返し、空白・タブ・改行記号、桁位置の縦線
- アプリ同梱の IBM Plex Sans JP（UI）と UDEV Gothic JPDOC（エディタ）を既定にした、プレビュー付きのフォント設定
- 未保存のまま終了でき、次回起動時に前回のタブと内容を復元
- 外部プログラムによるファイル変更の自動再読込、すべて保存、ディスクからの開き直し、カーソル行の選択
- 繰り返しの編集を覚えさせて流せるキーボードマクロ（名前を付けて保存できます）
- よく使う操作だけを置いたツールバーと、すべての機能を名前で引けるコマンドパレット
- JSON・XML/XAML・C#・JavaScript・TypeScript・CSSの文書全体を書式整形
- 項目を名前で絞り込める設定画面
...
1. ツールバーの「新しい文書」または「ファイルを開く」から文書を用意します。ウィンドウへのファイルのドロップでも開けます（複数選択可）。拡張子の制限はなく、拡張子なしも受け付けます。PDFは閲覧用、それ以外はテキストとして読み込みます。
2. 複数の文書を開いたときは、左側の垂直タブで切り替えます。
3. ツールバーには「新しい文書」「開く」「保存」「元に戻す」「やり直す」「検索」「設定」を置いています。それ以外の機能は、中央の「コマンドを名前で探す」（`Ctrl+Shift+P`）か、編集画面の右クリックメニューから選びます。
...
`.md` ファイルではツールバーのプレビューボタン、または `Ctrl+Shift+M` で編集画面とプレビューを切り替えられます。`.pdf` ファイルは読み取り専用のタブで開き、最初は表示領域の幅に合わせて表示します。倍率の隣にある「高さに合わせる」「幅に合わせる」で表示を切り替えられ、ウィンドウのサイズ変更やページ移動にも追従します。拡大・縮小ボタンや倍率表示（100% に戻す）を押すと固定倍率になります。前回のタブを復元するときは、終了時の表示方式を引き継ぎます。

約2GBを超えるテキストファイルは、Fumilumeが一度に読み込める上限を超えるため開けません。大容量確認の設定にかかわらず、読込前に理由を表示します。
...
- **開いているもの**: 開いている文書・PDF・検索結果・設定のタブ一覧です。設定以外はピンのボタンで一覧の先頭へ固定でき、セッション復元の対象になる文書・PDFではピン状態も次回起動時に復元されます。パネル幅はドラッグで 120px まで狭くできます。
...
左パネルと本文の境界をドラッグすると、パネル幅を176～480pxの範囲で変更できます。選んだ幅は次回起動時にも復元されます。
...
| Markdown / CSV プレビューを切り替える | `Ctrl+Shift+M` |
| 現在の文書を閉じる | `Ctrl+W` |
| 元に戻す / やり直す | `Ctrl+Z` / `Ctrl+Y` |
| 検索 / 置換 | `Ctrl+F` / `Ctrl+H` |
| 次 / 前の検索結果 | `F3` / `Shift+F3` |
| 指定行へ移動 | `Ctrl+G` |
| 文書全体を書式整形 | `Ctrl+K`, `Ctrl+D` |
...
開いた文書が UTF-8、UTF-8 BOM、UTF-16 LE／BE、UTF-32 LE／BE、Shift_JIS、EUC-JP、ISO-2022-JP のいずれかであれば、保存時も同じ文字コードを維持します。BOMなしのUTF-16／UTF-32も、NULの配置から安全に判定できる場合は読み込めます。改行コードも CRLF、LF、CR のまま保存されます。
...
Fumilume の起動中に別のファイルをダブルクリックした場合は、新しいウィンドウを増やさず、既存のウィンドウへ追加して開きます。同じファイルが既に開いている場合は、タブを増やさず、そのタブへ切り替えます。
...
- **文字化けする**: 対応している文字コードは UTF-8、UTF-8 BOM、UTF-16 LE／BE、UTF-32 LE／BE、Shift_JIS、EUC-JP、ISO-2022-JP です。自動判定できない文字コードのファイルは、あらかじめ対応形式へ変換してください。
- **書き込み中のログを開きたい**: 書き込み元のアプリが読み取りを許可していれば、そのアプリを終了せずに開けます。読み取りを許可していない場合は、書き込み元がファイルを閉じてから開いてください。
